package com.zsp.indexlib.IndexBar.widget;
...
import com.zsp.indexlib.IndexBar.helper.IIndexBarDataHelper;
import com.zsp.indexlib.IndexBar.helper.IndexBarDataHelperImpl;
...
public class IndexBar extends View {
    private static final String TAG = "zsp/IndexBar";
...
//        mPressedBackground = Color.BLACK;//默认按下是纯黑色
        mPressedBackground = Color.TRANSPARENT;//默认按下是透明
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        try {
            measureHeight = (int) (mIndexDatas.size() * indexBounds.height() * 2.5);
        } catch (Exception e) {

        }

        setMeasuredDimension(SizeUtils.dp2px(15), measureHeight);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndexDatas.size() > 0) {
            mPaint.setColor(Color.parseColor("#2396F5"));
            canvas.drawCircle((float) (mWidth / 2 - mPaint.measureText(mIndexDatas.get(choisePosition)) / 2 + SizeUtils.dp2px(13) / 3.5), t + mGapHeight * choisePosition + baseline - SizeUtils.dp2px(13) / 3, 20, mPaint);
            mPaint.setColor(Color.parseColor("#FFFFFF"));
            canvas.drawText(mIndexDatas.get(choisePosition), mWidth / 2 - mPaint.measureText(mIndexDatas.get(choisePosition)) / 2, t + mGapHeight * choisePosition + baseline, mPaint);//调用drawText，居中显示绘制index
        }
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (null != mOnIndexPressedListener && pressI > -1 && pressI < mIndexDatas.size()) {
                    Log.d("mOnIndexPressedListener", "mOnIndexPressedListener-------->" + pressI);
                    mOnIndexPressedListener.onIndexPressed(pressI, mIndexDatas.get(pressI));
                    choisePosition = pressI;
                    invalidate();
                }
                break;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mWidth = w;
        mHeight = h;
        //add by zsp 2016 09 08 :解决源数据为空 或者size为0的情况,
